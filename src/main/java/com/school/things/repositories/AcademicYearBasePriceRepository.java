...
public interface AcademicYearBasePriceRepository extends JpaRepository<AcademicYearBasePrice, Long> {
    ...
    @Query(value = "SELECT a.id, a.payment_item_id, a.payment_item_price, a.price_year, " +
            "(SELECT p.name FROM payment_item p WHERE p.id = a.payment_item_id) AS paymentItemName, a.grade_id " +
            "FROM academic_year_base_price a", nativeQuery = true)
    List<Object[]> findAllWithPaymentItemNameAndGrade();

    @Query(value = "SELECT a.id, a.payment_item_id, a.payment_item_price, a.price_year, " +
            "(SELECT p.name FROM payment_item p WHERE p.id = a.payment_item_id) AS paymentItemName, a.grade_id " +
            "FROM academic_year_base_price a WHERE a.id = ?1", nativeQuery = true)
    Object[] findByIdWithPaymentItemNameAndGrade(Long id);

    @Query(value = "SELECT a.id, a.payment_item_id, a.payment_item_price, a.price_year, " +
            "(SELECT p.name FROM payment_item p WHERE p.id = a.payment_item_id) AS paymentItemName, a.grade_id " +
            "FROM academic_year_base_price a WHERE a.grade_id = ?1", nativeQuery = true)
